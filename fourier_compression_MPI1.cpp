...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels, int rank, int nbp)
{
    ...
    std::complex<double>* X_loc = new std::complex<double>[ni*nj];
    std::complex<double>* X_gather = new std::complex<double>[ni * nj * nbp];

    std::fill(X_loc, X_loc +ni*nj, std::complex<double>(0.,0.));
    for( std::uint32_t k1 = 0; k1 < ni; ++k1 )
    {
        for (std::uint32_t k2 = 0; k2 < nj; ++k2)
        {
            for (std::uint32_t n2 = 0; n2 < ni; ++n2 )
            {
                ...
                {
                    std::complex<double> exp2(std::cos(-2 * pi * (n2 + ni * rank) * k2 / (height)), std::sin(-2 * pi * (n2 + ni * rank) * k2 / (height)));
                    std::complex<double> exp1(std::cos(-2*pi*n1*(k1 + ni * rank)/nj), std::sin(-2*pi*n1* (k1 + ni * rank) /nj));
                    X_loc[k1*nj+k2] += double(pixels[3*(n1+ (n2 + ni * rank)*nj)])*exp1*exp2;
                }
            }
        }
    }

    MPI_Allgather(X_loc, ni * nj, MPI_DOUBLE, X_gather, ni * nj, MPI_DOUBLE, MPI_COMM_WORLD);
